...
const Login = ({ onAuthStart, onAuthComplete ,handleFormClose,toggleRegisterForm}) => {
  const { login } = useAuth();
    ...
    <div className="flex items-center justify-center min-h-screen bg-gradient-to-r from-blue-500 via-indigo-600 to-purple-700">
      <div className="w-full max-w-sm bg-gray-900 text-white rounded-3xl p-8 shadow-lg transform hover:scale-105 transition-all duration-300">
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          <input type="email" value={email} onChange={(e) => setEmail(e.target.value)} placeholder="البريد الإلكتروني" required      className="w-full py-3 pl-10 pr-4 bg-gray-800 text-white border border-gray-700 rounded-lg focus:ring-2 focus:ring-indigo-500"
            />
          </div>
          <div className="relative">
            <FaLock className="absolute top-3 left-3 text-gray-400" />
               <input type="password" value={password} onChange={(e) => setPassword(e.target.value)} placeholder="كلمة المرور" required    className="w-full py-3 pl-10 pr-4 bg-gray-800 text-white border border-gray-700 rounded-lg focus:ring-2 focus:ring-indigo-500"
            />
          </div>
          <button
            type="submit"
