...
import { getExpensesCategories, createExpenseCategory, updateExpenseCategory, deleteExpenseCategory } from '../../services/api/GeneralReqApi';
...
const ExpenseCategorys = () => {
 ...
 triggerAlert('error', 'حدث خطأ في حذف نوع المصروف');
    ...
    const currentItems = expenseCategorys.slice((currentPage - 1) * itemsPerPage, currentPage * itemsPerPage);
                ...
                <div className="fixed inset-0 flex items-center justify-center bg-black bg-opacity-50 z-50">
                    <div className="bg-white dark:bg-gray-800 rounded-lg shadow-lg p-6 w-full max-w-lg">
                        <h3 className="text-xl font-semibold mb-4 text-gray-800 dark:text-gray-100">
                            {editingExpenseCategory ? 'تعديل نوع المصروف' : 'إضافة نوع المصروف'}
                        </h3>
                        <form
                            onSubmit={(e) => {
                                e.preventDefault();
                ...
                <table className="w-full border-collapse border border-gray-300 dark:border-gray-700">
                    ...
                    <tbody>
                        {currentItems.map((expenseCategory) => (
                            <tr key={expenseCategory.id} className="bg-white text-center dark:bg-gray-900 hover:bg-gray-100 dark:hover:bg-gray-700">
                                ...
                                <td className="px-4 py-2 border border-gray-300 dark:border-gray-600">
                                    <div className="flex items-center justify-center gap-6">
                                        ...
                                        <button
                                            onClick={() => handleDeleteExpenseCategory(expenseCategory)}
