...
import { createLegalAd, updateLegalAd, getLawyers, getCourts, getLegalAdTypes } from "../../../../services/api/legalCases";
import useAuth from "../../../auth/AuthUser";
import { useAlert } from "../../../../context/AlertContext";


const LegalAdModal = ({ isOpen, onClose, legCaseId, fetchLegalAds, initialData = {}, isEdit = false }) => {
...
updated_by: user.id
  ...
  useEffect(() => {
    const fetchDropdownData = async () => {
      try {
        const [courtResponse, legalAdTypeResponse, lawyersResponse] = await Promise.all([
          getCourts(),
          getLegalAdTypes(),
          getLawyers(),
        ]);
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (response && response.status === 200) {
        triggerAlert('success', isEdit ? 'تم تحديث الإعلان بنجاح.' : 'تم إضافة الإعلان بنجاح.');
        fetchLegalAds();
        onClose();
      } else {
          ...
          <div className="bg-white dark:bg-gray-900 rounded-2xl shadow-2xl w-full max-w-5xl p-8 relative transform transition-all overflow-auto max-h-[90vh]">
            ...
            <form onSubmit={handleSubmit} className="space-y-4">
              {[
                ...
                { label: "نوع الإعلان", name: "legal_ad_type_id", options: legalAdTypes },
                { label: "المحامي المرسل", name: "lawyer_send_id", options: lawyers }, 
              ].map(({ label, name, options }) => (
                <div key={name}>
                  <label className="block text-sm font-bold text-blue-700 dark:text-gray-200">{label}</label>
                  <select
                    name={name}
                    value={formData[name]}
              ...
              <div>
                <label className="block text-sm font-bold text-blue-700 dark:text-gray-200">الوصف</label>
                <input
                  name="description"
                  type="text"
              ...
              <div className="grid grid-cols-1 md:grid-cols-1 gap-4 items-center">
                <label className="text-sm font-medium text-gray-700 dark:text-gray-200">تاريخ التسليم</label>
                <input
                  name="send_date"
                  type="date"
                ...
                {isEdit && (
                  <>
                      <div >
                  <label className="block text-sm font-bold text-blue-700 dark:text-gray-200">  المحامي المستلم </label>
                  <select
                  ...
                  >
                    ...
                    <label className="text-sm font-medium text-gray-700 dark:text-gray-200">تاريخ الإستلام</label>
                    <input
                      name="receive_date"
                      type="date"
              ...
              {isEdit && (
                <>
                  <div>
                    <label className="block text-sm font-bold text-blue-700 dark:text-gray-200">النتائج</label>
                    <input
                      name="results"
                      type="text"
                  ...
                  <div>
                    <label className="block text-sm font-bold text-blue-700 dark:text-gray-200">الحالة</label>
                    <select
                      name="status"
              ...
              <button type="submit" className="w-full py-3 bg-gradient-to-r from-blue-500 to-indigo-600 text-white rounded-lg shadow-lg hover:shadow-xl transition-transform transform hover:-translate-y-1">
                {isEdit ? "تحديث" : "إضافة"}
              </button>
            </form>
          </div>
