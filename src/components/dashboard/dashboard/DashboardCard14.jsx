...
const ServiceItem = ({ service, isDarkMode }) => {
  ...
  return (
    <li className="flex items-center gap-4 p-3 rounded-lg transition-all duration-200 hover:bg-gray-200 dark:hover:bg-gray-700">
      <div className={`w-10 h-10 flex items-center justify-center rounded-full shrink-0 ${color}`}>
        <svg className="w-6 h-6 fill-current text-white" viewBox="0 0 24 24">
          <path d={icon} />
        </svg>
      </div>
      <div className={`flex-1 flex flex-col ${isDarkMode ? "text-gray-100" : "text-gray-800"}`}>
        ...
        <p className={`text-sm font-medium ${color === "bg-green-500" ? "text-green-600" : color === "bg-yellow-500" ? "text-yellow-600" : "text-red-600"}`}>
          {service.status === "completed" ? "مكتملة" : service.status === "in-progress" ? "قيد التنفيذ" : "معلقة"}
        </p>
      </div>
    </li>
  );
      ...
      <div className="p-3">
        <ul className="space-y-3">
          {services.map((service) => (
            <ServiceItem key={service.id} service={service} isDarkMode={isDarkMode} />
          ))}
        </ul>
      </div>
    </div>
