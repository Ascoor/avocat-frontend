...
const AddEditServiceModal = ({ show, handleClose, service, isEditing }) => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const [clientsResponse, unclientsResponse,serviceTypesResponse] = await Promise.all([
  ...
  useEffect(() => {
    if (isEditing && service) {
      setServiceData({
        ...service,
        client_choice: service.clients && service.clients.length > 0 ? 'client' : 'unclient',
        client_id: service.clients && service.clients.length > 0 ? service.clients[0].id : '',
        unclient_id: service.unclients && service.unclients.length > 0 ? service.unclients[0].id : '',
  ...
  const unclientOptions = unclients.map(unclient => ({ value: unclient.id, label: unclient.name }));
  ...
  <Modal.Body>
    ...
    <Form.Group controlId="client_choice">
      {isEditing ? (
        <Form.Label>{serviceData.client_choice === 'client' ? 'عميل' : 'عميل غير مسجل'}</Form.Label>
      ) : (
        <>
          <Form.Check
            type="radio"
    ...
    {serviceData.client_choice === 'client' && (
      <Form.Group controlId="client_id">
        ...
        <Select
          name="client_id"
          value={clientOptions.find(option => option.value === serviceData.client_id)}
    ...
    {serviceData.client_choice === 'unclient' && (
      <Form.Group controlId="unclient_id">
        ...
        <Select
          name="unclient_id"
          value={unclientOptions.find(option => option.value === serviceData.unclient_id)}
          onChange={handleClientChange}
